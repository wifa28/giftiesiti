            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active"><?php echo anchor('orders', 'Order'); ?></li>
                        ...
                        <div class="text-center actionRow">
                            <?php if ($data->payment_method == 1) : ?>
                                <?php if ($data->order_status == 1) : ?>
                                    ...
                                    <a href="<?php echo site_url('payments/confirm?order='. $data->id); ?>" class="btn btn-success">Konfirmasi Pembayaran</a>
                                    <br>
...
<?php if (($data->payment_method == 1 && $data->order_status == 1) || ($data->payment_method == 2 && $data->order_status == 1)) : ?>
    ...
    <script>
        $('.cancel-btn').click(function(e) {
            ...
            $.ajax({
                method: 'POST',
                url: '<?php echo site_url('orders/order_api?action=cancel_order'); ?>',
                data: {
                    id: <?php echo $data->id; ?>
                },
                context: this,
...
<?php if (($data->payment_method == 1 && ($data->order_status == 5 || $data->order_status == 4)) || ($data->payment_method == 2 && ($data->order_status == 4 || $data->order_status == 3))) : ?>
    ...
    <script>
        $('.delete-btn').click(function(e) {
            ...
            $.ajax({
                method: 'POST',
                url: '<?php echo site_url('orders/order_api?action=delete_order'); ?>',
                data: {
                    id: <?php echo $data->id; ?>
                },
                context: this,
                success: function(res) {
                    if (res.code == 200) {
                        ...
                        if (res.success) {
                            ...
                            setTimeout(() => {
                                window.location = '<?php echo site_url('orders'); ?>';
                            }, 5000);
                        } else if (res.error) {
                            $('.actionRow').html(res.message);
